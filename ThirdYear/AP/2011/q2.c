/*
2)a)
Conditional Critical regions are regions are cirtical regions, the access to which is controlled by condition variables. This is needed when access to the data is dependent on it's current state. For example, two teads with one writing "1" to the stdout buffer and the other writing "0", with both threads alternating which one writes.
*/
...
int main(int argc, char * argv[]){                 // Main function
    ...
    if(pthread_create(&t0, NULL, thread_0, NULL)){ // initilise the thread
        return 1;                                  // if failed, return 1
    }
    if(pthread_create(&t1, NULL, thread_1, NULL)){ // initilise the thread
        return 1;                                  // if failed, return 1
    }
    pthread_join(t0, NULL);                        // wait for thread 0 to complete
    pthread_join(t1, NULL);                        // wait for thread 1 to complelte
    return 0;                                      // return 0
}
